...
The code developing the inputs to the *fleetEffSim* model is available under the `~data-raw/model_data` file, and are broken up into two subfolders: `costs-inputs` and `benefits-inputs`. As the name suggests, the `cost-inputs` file runs through the creation of the cost curves, as well as te expected business as usual emissions trajectory. The `benefit-inputs` folder includes scripts for some assumptions in the benefit side of the model, including the fuel forecasts, km travelled by vehicles, energy prices, energy intesnity of the grid and EV energy consumption forecasts. 
...
This issue can also be thought about in terms of where each vehicle 'starts' on the ICE cost curves discussed earlier in this chapter. The *fleetEffSim* model treats existing technology in this way, assuming that the existing technology applied to vehicles is the lowest cost upgrades available. 
...
So how do we estimate this 'starting point' in the *fleetEffSim* model? 
...
The assumptions underpinning each of these variables is discussed below, and the code used to prepare the data for each assumption is available in the `data-raw/model_data/benefits-inputs` file.
...
The proportion of vehicles assumed to run on diesel is determined based on data from: https://fleetautonews.com.au/new-car-sales-by-fuel-type-fleet-buyers, and is dependent on vehicle type. 
This data is not perfect - it only includes data from fleet sales, not all vehicle sales. However, this account fro ~50% of all vehicles, and there is the best data available for 2008 sales. The arguments `.in_passenger_diesel`, `.in_suv_diesel` and `.in_lcv_diesel` specify the diesel proportions in each segment and can be changed by the user, however have the default arguments (as a proportion) of 0.02, 0.21 and 0.92 respectively. 
...
Chapter 2 outlined the assumptions used in the *fleetEffSim* model, and how those assumptions were developed. This chapter looks at the model itself, some assumptions that are built into the model, and how the model runs as a whole. 


## Structure of the *fleetEffSim* model

As the figure below shows, there are three main functions in the fleetEffSim model: `compliance_costs()`, `benefit_model()` and `generate_results()`. Each function also calls on smaller functions as they run. 

![The structure of the fleetEffSim model](Slide1.png)


When put together, a model run may consist of something similar to the following:
...
#now running the output of the compliance_cost() function through the benefit_model()
#function

central_benefit <- benefit_model(.fleet = central_compliant)

#Now similarly calling the compliance cost  and benefit functions for the BAU scenario, as both are required to run the `generate_results()` function:

bau_compliant <- compliance_costs(.fleet = fleet_creator(),
                                  .target_scenario = "bau")

bau_benefit <- benefit_model(.fleet = bau_compliant)

#Calling the `generate_results()` function to get hte overall results of the target scenario (relative to the BAU scenario)

results <- generate_results(bau_benefits = bau_benefit,
                            target_benefits = central_benefit,
                            cars = 100) #this is as fleet_creator() defaults to producing a fleet of 100 cars in the base year


```

It's clear that the cost and benefit models are typically run twice - once each for the target scenario being tested, and again for the business as usual scenario to compare this to, and then the results summarised by the generate_results() function. 

This structure is the underlying structure of the `fleet_eff_sim()` function, which simply combines the three functions outlined into a streamlined process to reduce the work required by the user. 




## Compliance costs function

The `compliance_costs()` function aims to determine the lowest cost pathway to achieve a set level of carbon dioxide emissions as specified by the target used as an input. This target could be a policy target, or the business as usual scenario that is expected to occur regardless of policy intervention. 

The `compliance_costs()` function consists of two (or three) main steps:

 0. We will call this step 0 as it's not technically in the `compliance_cost()` function, but the `fleet_creator()` function is typically used to produce a simulated fleet for the model to use. 
 1. The existing level of technology in the simulated fleet is set via the `add_existing_technology()` function. This determines which part of the technology cost curve is available to be applied to new vehicles. 
 2. Each year is then looped over, with the `select_upgrade()` function used to select, one at a time, the most cost effective technology option to apply to a vehicle to reduce its CO2 emissions (from the technology cost curves). After each addition, the average emissions of the fleet are calculated and compared to the target value. This loop continues until the target for a given year is met. It then moves to the next year, and repeats. 
 
 
 In this way, the `compliance_costs()` function sketches out a lowest cost path to meeting the specified emissions targets within each year. It records the technology added to every simulated vehicle sold in the test period, the costs of all upgrades added to the vehicles, as well as whether an upgrade is to an electric vehicle or not. This output is then directly used in the `benefit_model()` function
 
 
 
## Benefit model function
 
Once the `compliance_cost()` function has estimated the lowest cost pathway available to meeting emissions for a given set of vehicle sales and emissions targets, this output can be used in the `benefit_model()` function to determine what this means for the vehicles (and users) once they are on the road. In particular, this includes working out how far a given vehicle is likely to drive (based on it's age, type and running costs), how much petrol, diesel or electricity it is likely to use over those kilometres travelled, and the cost associated with that usage. 
 
When run for a target and business as usual scenario, the output can then be used to compare between the two scenarios -- to determine the difference in running costs under a no target and target scenario. 
 
 The `benefit_model()` can be broken into a few steps:
  1. Assigning fuel types to the vehicles sold
  2. Simulating the use of each vehicles sold over it's life
  3. Using the vehicle usage to calculate running costs and emissions
  
### Assigning fuel types to the vehicles sold

The vehicles sold in the Australian market are a mix of fuel types - notably between octane 91, 95 and 98 petrol, diesel, and electric. The *fleetEffSim* model assumes that even as many users switch to electric vehicles, the proportions of fuel types in the remaining ICE fleet remain constant. The `benefit_model()` therefore initially seperates the 'electric' and 'non-electric' parts of the fleet, and then applies the existing breakdown between fuel types to the 'non-electric' part of the fleet. 

### Simulating the use of each vehicle sold, over it's lifetime

Now that each vehicle has been assigned a fuel type, the `benefit_model()` function simulates the lifetime of each newly purchased vehicle from the output of the `compliance_cost()` function. This essentially consists of a loop that, for the period specified, sets the conditions and use of a vehicle based on what year it is, and the age/type of vehicle. For exaple, if targets begin in 2024, the year 2024 will be the first year of 'sales'. These vehicles will be 0 years old, and will have characteristics assigned to them based on that age and the fact that the year is 2024. The loop will then continue to 2025, where the vehicles sold in 2024 will have new entires added for their age being 1 - and new vehicles (with age 0) sold in 2025 will be added to the fleet of vehicles on the road. 

In doing this, the model first adds the assumed fuel and electricity prices based on the current year, as well as the assumed energy consumption of electric vehicles and the emissions intensity of the grid. The kilometres driven by each vehicle is then assigned based on the vehicles age and type, and running costs (via a rebound effect). A rebound effect is included to account for the fact that when running costs become cheaper, vehicles tend to be driven further (an elasticity of 0.1 is used). 

### Using vehicle characteristics to calculate running costs and emissions

Once all the vehicle specific characteristics are included, it is reasonably simple to create a series of new columns which calculate the figures we're interested in - running costs and emissions. 

These are calculated based on each vehicle characteristic, and the conversion from emissions data to fuel consumption for each vehicle is calculated through the functions `petrol_co2_to_fuel()` and `diesel_co2_to_fuel()`.


## Generate results function

Once the `compliance_costs()` and `benefit_model()` functions have been run for the target and business as usual scenarios, the `generate_results()` function used to compare the two, and calculated the additional costs and benefits under the target scenario. It also applies various discount rates to the data and includes this in the output. 

The `benefit_model()` function first summarises the results of the benefit model for both the target and BAU scenarios, excluding all vehicles that are assumed to be dead (that is vehicles that are over 17 years of age, the assumed vehicle lifetime). It also adjusts the figure to represent the proportions of the real vehicle fleet, as opposed to the simulated fleet. This is done by assuming that the base year (2021, as created by the fleet_creator()) has a real sales figure of approximately 1,100,000 vehicles. As the simulated fleet most commonly used has a set base year number of cars, each therefore represents 1,100,000/(base year fleet size).  

The business as usual scenario is then subtracted from the target scenario to determine the additional benefits of the target policy, and the simplified results are then discounted through the `discount()` function, at three different discount rates.
