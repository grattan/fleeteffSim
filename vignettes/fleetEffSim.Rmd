...
In it's most simple form, the fleetEffSim model can be run using a single function `fleet_eff_sim()`, which takes arguments to specify each of assumptions the user would like to make. Although in reality the model is broken up into two distinct parts - a 'cost' model, and a 'benefit' model, the `fleet_eff_sim()` function combines the two and produces summarised results of a model run (each of the two parts of the model can be accessed seperately to provide more granular, less summarised outputs. This is discussed in chapter 3). 

This chapter will be brief, but will give an overview of the inputs to the `fleet_eff_sim()` function, as well as the output produced. 


## Function inputs 

The `fleet_eff_sim()` function takes 27 arguments -- however, all of these arguments have a default specification, and many are unlikely to be changed (provided the user is happy with the assumptions they specify). All arguments are detailed in the function documentation, however some key parameters a user might wish to change are discussed below. 


### Using the inbuilt scenarios

Although the `fleet_eff_sim()` takes many arguments, the model can be run most simply by changing very few arguments, particularly if the scenarios used are included in the `fleetEffSim` package. This particularly relates to the targets tested. 

Targets in the `fleet_eff_sim()` function are specified through the `.in_target_file` and `in.target_scenario` arguments. By default, the `.in_target_file` is set to `targets_and_bau` - a \code{tibble} containing 3 target scenarios and one business as usual scenario. The three target scenarios are: target_central, target_linear, and target_ambitious, and the business as usual scenario is called bau. The user can specify which of the scenarios is run in the model through the `.in_target_scenario` variable. By default, the `.in_bau_scenario` is set to the bau scenario in the target input file, and the `.in_target_scenario` argument is set to `target_central`.

For example, to run the linear scenario, the following code could be used:

```{r, eval = FALSE}
output <- fleet_eff_sim(
...
### Evaluating emissions targets that are not included in the fleetEffSim model: the `.in_target_file`, `.in_target_scenario` and `.in_bau_scenario` arguments

Although the `fleet_eff_sim()` model does include three target scenarios, users may wish to test a target that is not include in the default data. This can be done through using the `.in_target_file`, `.in_target_scenario`, and `.in_bau_scenario` arguments. 

The `.in_target_file` can be specified by the user, but must contain:

 - three columns, named `year`, `target_type`, and `value`. 
 - a bau and non bau scenario (the bau scenario should be called `bau` under the `target_type` variable) which have equivalent years specified

The `value` argument specifies the emissions target or, for the bau scenario, the expected average emissions for a given year, in gCO2/km.

The user must also specify through the `.in_target_scenario` argument the `target_type` variable of the scenario they would like to run in the model. This is the case regardless of how many scenarios are included in the `.in_target_scenario` file. 



### Using your own input fleet: `.fleet_creator`, `.input_fleet` and `.in_cars`

The `.fleet_creator` argument defaults to TRUE, and specifies whether the user would like the model to user an inbuilt function `fleet_creator()` to produce a simulated fleet for the model to use (in this case  `.fleet_creator = TRUE`), or whether the user would like to specify their own simulated fleet for the model to use. This might be the case if the user wants to test a scenario where the vehicle sales proportions of different vehicle types (SUVs, passenger vehicles and LCV's) varies from the assumptions built into the `fleet_creator()` function.

In this case, the user should build a simulated fleet to the same specifications are that produced by the `fleet_creator()` function (which can be found by calling `fleet_creator()`). A fleet of 100 cars in the base year is recommended (each 'vehicle' represents 1% of the vehicle fleet). 

The argument `.input_fleet` should be used to specify the users fleet, `.fleet_creator` should be set to FALSE, and the`.in_cars` variable to be set to the number of cars in the users simulated fleet in the base (first) year. 
